...
parse_kofam_detail <- function(path) {
  ...
  gene <- apply(df, 1, function(r) {
    idx <- which(!is.na(r))[1];
    if (length(idx)) r[idx] else NA_character_
  })
...
parse_species <- function(label, mapper_file, detail_file) {
...
# Highlights nodes/edges in a processed KEGG graph (ggkegg::process_line() output) that correspond to the provided KOs, derives pathway titles touching those edges
...
# Get all pairs of files in the current directory
file_pairs <- list.files(pattern = "_mapper.txt$")
file_pairs <- gsub("_mapper.txt$", "", file_pairs)

counts <- list()
for (prefix in file_pairs) {
  label <- prefix
  mapper_file <- paste0(prefix, "_mapper.txt")
  detail_file <- paste0(prefix, ".txt")
  ...
  g2k <- parse_species(label, mapper_file, detail_file)
  kos <- g2k$KO |> stats::na.omit() |> unique()
  
  out_png <- file.path("metabolic_maps", paste0("ko01100_glow_pathway_labels_", gsub("\\s+", "_", label), ".png"))
